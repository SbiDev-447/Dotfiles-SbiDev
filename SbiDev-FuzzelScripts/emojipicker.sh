...
# --- Seleccionar emoji ---
select_emoji() {
    local emoji_data_start
    emoji_data_start=$(grep -n "^### DATA ###$" "$0" | cut -d: -f1)
    
    if [[ -z "$emoji_data_start" ]]; then
        echo "Error: No se encontró la marca '### DATA ###' en el script." >&2
        exit 1
    fi
